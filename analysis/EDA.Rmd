...
reedy = relevant_tide_dat[, c('ReedyPoint.date', 'ReedyPoint.sea_level')] %>% 
  group_by(ReedyPoint.date) %>% 
  summarise(reedy_sl = mean(ReedyPoint.sea_level))


setnames(atlantic, 'AtlanticCity.date', 'date')
setnames(reedy, 'ReedyPoint.date', 'date')

compare = merge(reedy, atlantic)
...
extended_reedy = as.data.table(rbind(approx_reedy[, c('date', 'reedy_sl')], reedy))
