...
#chester <-  read_delim("../data/raw/chester_sc.txt", delim = "\t", skip = 49, col_names = FALSE)  
...
# Build Training Data
```{r}
# data sets to merge: tide: 
#[meyer_tide_dat]
#inflows: [dr, sr]
#sc: [chester, reedy, bfb])


dr <- dr %>% mutate(inflow = avg_discharge_cfps*0.0283168, river = 'delware')
sr <- sr %>% mutate(inflow = avg_discharge_cfps*0.0283168, river = 'schuylkill')

inflows <- rbind(dr_clean[, c('date', 'inflow', 'river')], sr_clean[, c('date', 'inflow', 'river')])

chester <- chester %>% mutate(location = 'chester')
reedy <- reedy %>% mutate(location = 'reedy')
bfb <- bfb %>% mutate(location = 'ben_franklin')


sc = rbind(chester[, c('date', 'location', 'mean_sc')], reedy[, c('date', 'location', 'mean_sc')], bfb[, c('date', 'location', 'mean_sc')])


tide_dat = tide_data[, c('AtlanticCity.date', 'AtlanticCity.sea_level')] %>% 
  group_by(AtlanticCity.date) %>% 
  summarise(avg_daily_sea_level = mean(AtlanticCity.sea_level))

setnames(tide_dat, 'AtlanticCity.date', 'date')
tide_dat$date = as.Date(tide_dat$date)


training_data = merge(inflows, tide_dat, how = 'left')
training_data = merge(training_data, sc, how = 'left')

```

```{r}
ggplot(training_data, aes(date, inflow, color = river))+
  geom_point()+
  facet_wrap(~river, scales = 'free_y', switch = 'y')+
  ggtitle('All Time Inflow Values')
```


```{r}
ggplot(training_data, aes(date, mean_sc, color = location))+
  geom_point()+
  facet_wrap(~location, , scales = "free_y", switch = 'y')+
  ggtitle("All Time Mean SC Values")
```
```{r}
ggplot(training_data, aes(date, avg_daily_sea_level))+
  geom_point()+
