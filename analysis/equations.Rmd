...
for (t in (2:nrow(input_data))){
  ...
  k_st <- st_k_values[t-1] + min(Ls, ds*(t_0$power_10d - st_k_values[t-1]) + tao_s/t_0$q_st*t_0$water_level) # reedy_sl uses our water levels, switch to water_level to use meyer values
  
  # set k_lt equal to equation 3
  k_lt <- lt_k_values[t-1] + min(Ll, dl*(t_0$power_30d - lt_k_values[t-1]) + tao_l/t_0$q_st*t_0$water_level) # reedy_sl uses our water levels, switch to water_level to use meyer values
...
```{r}
library(ggplot2)
copycat$date = as.Date(copycat$date)


ggplot()+
  geom_point(data = copycat, aes(x = date, y = mean_sc, color = 'mean_sc')) +
  geom_line(data = copycat, aes(x = date, y = model_output, color = 'pbem_model')) +
  xlab('Date') +
  ylab('SC Value')+
...
drill_down <- copycat %>% filter(date >= '1997-01-01' & date <= '1999-09-01')
