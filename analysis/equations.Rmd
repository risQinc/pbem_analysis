...
input_data <- dat %>% group_by(date) %>% 
  summarise(inflow = sum(inflow, na.rm = TRUE)/0.0283168, #convert inflow from cmps to cfps
                      reedy_sl = mean(reedy_sl)*12) # convert reedy_sl from inches to feet
```
...
input_data <- input_data %>% mutate(water_level = (reedy_sl - 566)/(12*9),
                      q_st = rollmean(x = inflow, n_s, align = 'right', fill = NA), # n_s = 10 days
                      q_lt = rollmean(x = inflow, n_l, align = 'right', fill = NA), # n_l = 30 days
                      power_10d = as*lag(q_st)^(Ps) + Cs,
                      power_30d = al*lag(q_lt)^(Pl) + Cl,
                       # need to ask Eliot about this
                      Q1 = Q1,
                      Q2 = Q2,
                      Ls = Ls,
                      Ll = Ll)
...
input_data <- input_data %>% drop_na(q_lt) %>%
          filter(date >= '1964-11-15') %>%
          fill(water_level) ## IS THIS A VALID IMPUTATION METHOD? (fill NA with previous value?)
...
# column E: 10d MA sum = q_st
# column F: irrelevant for BFB
# column G: 30d MA sum = q_lt
# column H: 10d Power Eqn = as* q_st - 1^Ps + Cs
# column I: 30d Power Eqn = al * q_lt - 1 ^ Pl + Cl
# column J: 10d Model = k_st
# column K: 30d Model = k_lt
...
for (t in (2:nrow(input_data))){
  ...
  k_st <- st_k_values[t-1] + min(Ls, ds*(t_0$power_10d - st_k_values[t-1]) + tao_s/t_0$q_st*t_0$water_level)
  
  # set k_lt equal to equation 3
  k_lt <- lt_k_values[t-1] + min(Ll, dl*(t_0$power_30d - lt_k_values[t-1]) + tao_l/t_0$q_st*t_0$water_level)
...
input_data <- input_data %>% mutate(model_output = model_10d*max(0, min(1, q_lt - thresh_30/ 700)) + 
                                      model_30d*(min(1, max(0, (thresh_10 - q_lt)/700) ))
                                    )
```
...
```{r}
validate = merge(input_data, bfb[, c('date', 'mean_sc')])
```
...
validate = validate %>% filter(inflow > 0)


ggplot()+
  geom_point(data = validate, aes(x = date, y = mean_sc, color = 'mean_sc')) +
  geom_line(data = validate, aes(x = date, y = model_output, color = 'pbem_model')) +
  xlab('Date') +
  ylab('SC Value')+
...
drill_down <- validate %>% filter(date >= '1998-01-01' & date <= '1999-09-01')
